...
def test_q_matches_description(client: TestClient):
    response = client.get("/products/?q=wireless")
    data = response.json()
    assert response.status_code == 200
    assert len(data) == 1
...
def test_q_no_match(client: TestClient):
    response = client.get("/products/?q=zzznomatch")
    assert response.status_code == 200
    assert response.json() == []


def test_sku_partial_match(client: TestClient):
    response = client.get("/products/?sku=SKU-00")
    assert response.status_code == 200
    assert len(response.json()) == 4


def test_sku_exact_match(client: TestClient):
    response = client.get("/products/?sku=SKU-001")
    data = response.json()
    assert response.status_code == 200
    assert len(data) == 1
...
def test_min_price(client: TestClient):
    ...
    data = response.json()
    assert response.status_code == 200
    assert {p["title"] for p in data} == {"Laptop", "Monitor"}
...
def test_max_price(client: TestClient):
    ...
    data = response.json()
    assert response.status_code == 200
    assert {p["title"] for p in data} == {"Mouse"}


def test_price_range(client: TestClient):
    response = client.get("/products/?min_price=50&max_price=500")
    data = response.json()
    assert response.status_code == 200
    assert {p["title"] for p in data} == {"Keyboard", "Monitor"}


def test_price_range_inclusive(client: TestClient):
    response = client.get("/products/?min_price=29.99&max_price=29.99")
    data = response.json()
    assert response.status_code == 200
    assert len(data) == 1
    assert data[0]["title"] == "Mouse"
...
def test_category_filter_no_match(client: TestClient):
    response = client.get("/products/?category_id=99999")
    assert response.status_code == 200
    assert response.json() == []


def test_has_image_true(client: TestClient):
    response = client.get("/products/?has_image=true")
    data = response.json()
    assert response.status_code == 200
    assert {p["title"] for p in data} == {"Laptop", "Keyboard"}


def test_has_image_false(client: TestClient):
    response = client.get("/products/?has_image=false")
    data = response.json()
    assert response.status_code == 200
    assert {p["title"] for p in data} == {"Mouse", "Monitor"}


def test_combined_q_and_has_image(client: TestClient):
    response = client.get("/products/?q=keyboard&has_image=true")
    data = response.json()
    assert response.status_code == 200
    assert len(data) == 1
...
def test_combined_price_range_and_has_image(client: TestClient):
    response = client.get("/products/?min_price=50&max_price=500&has_image=true")
    data = response.json()
    assert response.status_code == 200
    assert {p["title"] for p in data} == {"Keyboard"}


def test_combined_category_and_price(client: TestClient, category: Category):
    response = client.get(f"/products/?category_id={category.id}&max_price=100")
    data = response.json()
    assert response.status_code == 200
    assert {p["title"] for p in data} == {"Mouse", "Keyboard"}
